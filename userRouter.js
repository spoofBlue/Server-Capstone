...
router.put(`/:id`, jwtAuth, jsonParser, (req, res) => {
  ...
  Users.findByIdAndUpdate(req.params.id, { $set: toUpdate })
...
function checkPutRequestForErrors(req) {
  ...
  explicitlyTrimmedFields.forEach(field => {
    if (field in req.body && req.body[field].trim() !== req.body[field]) {
      errorMessage.push(
        `The field ${field} cannot not start or end with whitespace.`
      );
    }
  });
  ...
  Object.keys(sizedFields).forEach(field => {
    if (
      ...
      errorMessage.push(
        `The field ${field} must be more than ${
          sizedFields[field].min
        } characters.`
      );
    }
  ...
  Users.find({ username: req.body.username })
    ...
    .then(count => {
      if (count > 0) {
        errorMessage.push(
          `That username is already taken. The username must be unique.`
        );
      }
    })
    .catch(err => {
